...
static const uint16_t CMD_FLASH = ('~' << 8) | 'F';
...
typedef enum {
  ...
  WAIT_STATUS_MASK_REBOOT = 1 << 4,
  WAIT_STATUS_MASK_SHUTDOWN = 1 << 5,
  WAIT_STATUS_MASK_SLEEP = 1 << 6,
} WaitStatusMask;
...
typedef enum {
  ...
  WAIT_STATUS_FLASH_READY     = 0xFF,                           // awaiting new firmware to be flashed
} WaitStatus;
...
static uint8_t _rgb_led_status_dimming_level = 1;

static RgbColor _rgb_led_colors[8];
static uint8_t _rgb_led_dimming_level = 1;
...
void loop() {
  if (_wait_status == WAIT_STATUS_FLASH_READY) {
    return;
  }
  ...
  if (process_external_status_changes()) {
    rgb_led_update_all();
  }
...
int8_t process_command(const uint16_t cmd, const uint8_t payload[], const uint8_t payload_length) {
  ...
  if (cmd == CMD_RGB_LED_COLOR_SET || cmd == CMD_RGB_LED_BREATH_SET) {
    // Reset temporary color storage array first
    memset(_rgb_led_colors_new_set, 0, sizeof(_rgb_led_colors_new_set));

    if (!process_token_chunks(cmd, token_chunks, token_chunks_count, cmd_rgb_led_color_user_set)) {
      return ERR_INVALID_PARAMETERS;
    }

    bool is_different_color = _rgb_led_colors_new_mode != _rgb_led_colors_user_mode;
...
bool cmd_rgb_led_color_user_set(const uint16_t cmd, const uint8_t token_chunk_index, const uint16_t *tokens, const uint8_t tokens_count) {
  if (token_chunk_index >= sizeof(_rgb_led_colors_new_set) / sizeof(_rgb_led_colors_new_set[0])) {
    return false;
  }
  ...
  _rgb_led_colors_new_mode = RGB_LED_USER_MODE_CONSTANT_COLOR;
  return true;
}
  ...
  for (uint8_t i = 0; i < sizeof(_melody_notes) / sizeof(_melody_notes[0]); i++) {
    ...
    if (play_time >= prev_note_end_play_time) {
      ...
      if (i > _melody_note_playing_index) {
        ...
        _is_melody_quick_playing = _melody_notes[i].length < 50;
      } else {
...
void display_page_hdd_cage_voltage() {
  ...
  char line2[] = "00.00v  /  0.00v";

  uint8_t int_part_12v = (uint8_t)_disk_voltage_12v;
  uint8_t dec_part_12v = (_disk_voltage_12v - (float)int_part_12v) * 100;
  itoar(int_part_12v, &line2[1]);
  itoar(dec_part_12v, &line2[4]);

  uint8_t int_part_5v = (uint8_t)_disk_voltage_5v;
  uint8_t dec_part_5v = (_disk_voltage_5v - (float)int_part_5v) * 100;
  itoar(int_part_5v, &line2[11]);
  itoar(dec_part_5v, &line2[14]);
...
bool display_message(const __FlashStringHelper *s1, const __FlashStringHelper *s2, bool is_alert) {
  ...
  line1[strcopy(line1, s1, DISPLAY_WIDTH)] = '\0';
  line2[strcopy(line2, s2, DISPLAY_WIDTH)] = '\0';
...
void display_wait_screen() {
  ...
  } else if (_wait_status == WAIT_STATUS_BOOT_PHASEN) {
  ...
  } else if (_wait_status == WAIT_STATUS_REBOOT_PHASE1 || _wait_status == WAIT_STATUS_SHUTDOWN_PHASE1) {
    if (display_page_texts[0] && display_page_texts[0][0]) {
      strcopy(line2, &display_page_texts[0][0], DISPLAY_WIDTH);
    } else {
      strcopy(line2, F("Stopping System"), DISPLAY_WIDTH);
    }
...
void rgb_led_restore_color() {
  ...
  if (_rgb_led_colors_user_mode == RGB_LED_USER_MODE_NONE) {
    rgb_led_set_colors(&RGB_GREEN, 1, false);
    return;
  }
...
void rgb_led_set_colors(const RgbColor *colors, const uint8_t count, const bool breathing_mode) {
  ...
  float r = (float)_rgb_led_colors[0].r / divider;
  float g = (float)_rgb_led_colors[0].g / divider;
  float b = (float)_rgb_led_colors[0].b / divider;
  
  if (_rgb_led_colors[0].r != 0 && r < 1) { r = 1; }
...
void rgb_led_update(const unsigned long elapsed_time) {
  ...
  if (_rgb_led_colors[_rgb_led_toggle_counter].length == 0) {
    _rgb_led_next_toggle_time = 0;
    _rgb_led_toggle_counter = 0;
    return;
  }

  _rgb_led_next_toggle_time = elapsed_time + _rgb_led_colors[_rgb_led_toggle_counter].length;
...
void rgb_led_update_all() {
  rgb_led_update_all(_rgb_led_breath_direction != 0 ? _rgb_led_colors[2] : _rgb_led_colors[_rgb_led_toggle_counter]);
}
...
uint8_t strcopy(char *dst, const char *src, const uint8_t max_length) {
...
uint8_t strcopy(char *dst, const __FlashStringHelper *src, uint8_t max_length) {
  if (dst == nullptr || src == nullptr || max_length == 0) {
    return 0;
  }
